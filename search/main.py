...
def train_parallel(l,iter):
    with open('train_parallel.sh','w') as f:
        for i in l:
            f.write('python train.py --save=nets/%s --iter=%d &\n'%(i,iter))
        f.write('wait')
...
def preprocess(dir):
    ...
    e2_nets=[k for k,v in epochs.items() if v==2]
    e3_nets=[k for k,v in epochs.items() if v==3]
    return history,accuracy,last_updated,best_acc,epochs,rank(e1_nets,best_acc),rank(e2_nets,best_acc),rank(e3_nets,best_acc),nets_index
...
def main():
    ...
    def mother_choose(e1_nets,e2_nets,e3_nets):
        l1,l2,l3=len(e1_nets),len(e2_nets),len(e3_nets)
        choice1,choice2,choice3=[],[],[]
        if l1:choice1=sample(l1,l1//4+1,6)
        if l2:choice2=sample(l2,l2//4+1,5)
        if l3:choice3=sample(l3,l3//4+1,5)
        return [e1_nets[i] for i in choice1] + \
               [e2_nets[i] for i in choice2] + \
               [e3_nets[i] for i in choice3]
    
    def mother2children(e1_nets,e2_nets,e3_nets,start):
        mothers=mother_choose(e1_nets,e2_nets,e3_nets)
    ...
    def dead_filter(e1_nets,e2_nets,e3_nets,history,iter):
        e1_nets=[net for net in e1_nets if iter-history[net]<10]
        e2_nets=[net for net in e2_nets if iter-history[net]<10]
        e3_nets=[net for net in e3_nets if iter-history[net]<10]
        return e1_nets,e2_nets,e3_nets
    f_record=open('evo_record','w')

    for iter in range(100):
        history,accuracy,last_updated,best_acc,epochs,e1_nets,e2_nets,e3_nets,nets_index=preprocess('nets')
        e1_nets,e2_nets,e3_nets=dead_filter(e1_nets,e2_nets,e3_nets,last_updated,iter)
        continued_nets=continue_choose(e1_nets,e2_nets)
        mothers,children=mother2children(e1_nets,e2_nets,e3_nets,max(nets_index))
        f_record.write('iteration: %d:\ncontinue: %s\nmutate:\n%s->%s\n'%(iter,str(continued_nets),str(mothers),str(children)))
        train_parallel(continued_nets+children,iter)
